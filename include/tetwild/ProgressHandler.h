...
namespace tetwild
{
	...
	public:
		...
		static void Trace(const char* fmt, FmtArgs&&... args)
		{
			...
			{
				s_ProgressHandler->DoMessage(spdlog::level::trace, FormatToString(fmt, std::forward<FmtArgs>(args)...));
			}
		}
		...
		static void Debug(const char* fmt, FmtArgs&&... args)
		{
			if (s_ProgressHandler)
			{
				s_ProgressHandler->DoMessage(spdlog::level::debug, FormatToString(fmt, std::forward<FmtArgs>(args)...));
			}
		}
		/// <summary>
		...
		static void Info(const char* fmt, FmtArgs&&... args)
		{
			if (s_ProgressHandler)
			{
				s_ProgressHandler->DoMessage(spdlog::level::info, FormatToString(fmt, std::forward<FmtArgs>(args)...));
			}
		}
		...
		static void Warn(const char* fmt, FmtArgs&&... args)
		{
			if (s_ProgressHandler)
			{
				s_ProgressHandler->DoMessage(spdlog::level::warn, FormatToString(fmt, std::forward<FmtArgs>(args)...));
			}
		}
		...
		static void Error(const char* fmt, FmtArgs&&... args)
		{
			if (s_ProgressHandler)
			{
				s_ProgressHandler->DoMessage(spdlog::level::err, FormatToString(fmt, std::forward<FmtArgs>(args)...));
			}
		}
		...
		static void Critical(const char* fmt, FmtArgs&&... args)
		{
			if (s_ProgressHandler)
			{
				s_ProgressHandler->DoMessage(spdlog::level::critical, FormatToString(fmt, std::forward<FmtArgs>(args)...));
			}
		}
		...
		static bool ShouldContinue()
		{
			if (s_ProgressHandler)
			{
				return s_ProgressHandler->DoShouldContinue();
			}
	...
	protected:
		...
		/// </summary>
		///	<remarks>
		/// Override this method in your own progress handler implementations to enable canceling the operation.
		///	The default implementation does not allow operations to be canceled.
		///	</remarks>
		/// <returns>`true` to continue the current operation, or `false` to cancel the current operation.</returns>
		virtual bool DoShouldContinue() const
		{
			return true;
		}
	...
	private:
		...
		/// </summary>
		static std::shared_ptr<ProgressHandler> s_ProgressHandler;
	};
